...
if skip_red == False:
    ...
    sci_location = []
    if float(scinum) < 10:
        sci_location= str(dloc)+"k.000"+str(scinum)+".fits"
    if float(scinum) >= 10:
        sci_location = str(dloc)+"k.00"+str(scinum)+".fits"
    ...
    def fit_cent_gaussian(row, clr):
        ...
        amp_guess = np.max(y_vals) - np.median(y_vals_bck) # Peak height - background
